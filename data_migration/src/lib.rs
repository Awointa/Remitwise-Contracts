...
mod tests {
    ...
    fn test_algorithm_field_roundtrips_json() {
        let snapshot = ExportSnapshot::new(sample_remittance_payload(), ExportFormat::Json);
        let bytes = export_to_json(&snapshot).unwrap();
        let loaded: ExportSnapshot = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(loaded.header.hash_algorithm, ChecksumAlgorithm::Sha256);
    }
    ...
    fn test_algorithm_field_roundtrips_binary() {
        ...
        let bytes = export_to_binary(&snapshot).unwrap();
        let loaded: ExportSnapshot = bincode::deserialize(&bytes).unwrap();
        assert_eq!(loaded.header.hash_algorithm, ChecksumAlgorithm::Sha256);
    }
    ...
    fn test_e2e_savings_snapshot_json_roundtrip() {
        ...
        let snapshot = build_savings_snapshot(export, ExportFormat::Json, 1);
    ...
    fn test_e2e_savings_snapshot_binary_roundtrip() {
        let export = make_single_goal_export();
        let snapshot = build_savings_snapshot(export, ExportFormat::Binary, 1);
    ...
    fn test_e2e_savings_snapshot_encrypted_roundtrip() {
        let export = make_single_goal_export();
        let snapshot = build_savings_snapshot(export, ExportFormat::Encrypted, 1);
        assert!(snapshot.verify_checksum());
    ...
    fn test_e2e_tampered_checksum_import_fails() {
        let export = make_single_goal_export();
        let mut snapshot = build_savings_snapshot(export, ExportFormat::Json, 1);
    ...
    fn test_e2e_locked_goal_preserved_json_and_binary() {
        ...
        // JSON path.
        let json_snapshot = build_savings_snapshot(export.clone(), ExportFormat::Json, 1);
        ...
        // Binary path.
        let bin_snapshot = build_savings_snapshot(export, ExportFormat::Binary, 1);
    ...
    fn test_e2e_snapshot_checksum_is_deterministic() {
        ...
        let snap_a = build_savings_snapshot(export_a, ExportFormat::Json, 1);
        let snap_b = build_savings_snapshot(export_b, ExportFormat::Json, 1);
    ...
    fn test_e2e_build_savings_snapshot_format_label() {
        ...
        for (format, expected_label) in formats {
            let export = make_single_goal_export();
            let snapshot = build_savings_snapshot(export, format, 1);
            assert_eq!(
