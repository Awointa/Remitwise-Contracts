...
fn test_multi_contract_user_flow() {
    ...
    let bill_id = bills_client.create_bill(
        ...
        &None, &SorobanString::from_str(&env, "XLM"),
    );
...
fn test_multiple_entities_creation() {
    ...
    let policy2 = insurance_client.create_policy(
        ...
        &CoverageType::Liability, &50i128, &10_000i128,
    );
...
fn test_event_topic_compliance_across_contracts() {
    ...
    for ev in events.iter() {
        ...
        let ok = topics.len() == 4 && 
                 soroban_sdk::Symbol::try_from_val(&env, &topics.get(0).unwrap()).is_ok() &&
                 soroban_sdk::Symbol::try_from_val(&env, &topics.get(0).unwrap()).unwrap() == symbol_short!("Remitwise");
        if !ok {
    ...
    assert_eq!(non_compliant.len(), 0u32, "Found {} events that do not follow the Remitwise topic schema. See the output above for details.", non_compliant.len());
}
